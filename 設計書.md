...
- **エラーテキスト入力**：
  - 「Error Message」タブで大型テキストボックスを提供
...
- **マージ動作**：  
  - 「Merge」クリック時：
    ...
    2. ファイルリストとエラーテキストの両方が空ならエラーメッセージ表示してreturn
    3. `merger.py`内の`merge_files()`関数呼び出し  
       引数：`project_dir`, `list_of_relative_paths`, `output_md_path`, `error_message`
...
- 関数：`merge_files(project_dir: str, relative_paths: List[str], output_md: str, error_message: str = None) -> None`
  1. **入力ファイル検証**：  
...
- 関数：`generate_dir_structure(project_dir: str, max_depth: int = 3) -> str`
  - `os.walk`や`pathlib`でプロジェクトディレクトリを走査  
...
3. **リセット**：  
   - `Reset`後、全ての入力（ファイルリスト、Entry、エラーテキスト）が空になり、ステータスもクリアされるか
